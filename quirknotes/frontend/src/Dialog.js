...
function Dialog({ open, initialNote, closeDialog, postNote: postNoteState, patchState }) {
    ...
    const patchNote = async (entry) => {
        ...
        try {
            await fetch(`http://localhost:4000/patchNote/${note._id}`, {
                ...
                if (!res.ok) {
                    setStatus("Error during edit of note");
                    console.log("Server failed:", res.status);
                } else {
                    await res.json().then(() => {
                        patchState(note._id, note.title, note.content);
                        setStatus("Note edited");
                    })
                }
            })
        } catch (error) {
            setStatus("Failed to contact server");
            console.log("Fetch patchNote failed:", error)
        }
    }
