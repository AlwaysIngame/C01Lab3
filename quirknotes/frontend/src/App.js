...
function App() {
  ...
  const deleteNote = async (entry) => {
    try {
      ...
      console.log("Failed to fetch deleteNote:", error);
  ...
  const deleteAllNotes = async () => {
    try {
      await fetch("http://localhost:4000/deleteAllNotes", {
        ...
        if (!response.ok) {
          console.log("Server Delete all failed:", response.status);
        } else {
          deleteAllNotesState()
        }
      });
    } catch (error) {
      console.log("failed to fetch deleteAllNotes:", error);
    }
  };
